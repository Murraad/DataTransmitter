    ...
    public class Program
    {
        ...
        static async Task Main()
        {
            try
            {
                ...
                processor = client.CreateProcessor(appSettings.QueueName, new ServiceBusProcessorOptions() { AutoCompleteMessages = false });

                //create processor for dead messages
                deadMessagesProcessor = client.CreateProcessor(appSettings.QueueName, new ServiceBusProcessorOptions() { AutoCompleteMessages = false, SubQueue = SubQueue.DeadLetter });
            ...
            if (blobClient.Exists())
            {
                ...
                if (content != null && content.GetRawResponse().Status == 200)
                {
                    ...
                    using (var form = new MultipartFormDataContent())
                    {
                        ...
                        var postResult = await httpClient.PostAsync(appSettings.ProcessPath, form);
                        if (postResult.IsSuccessStatusCode)
                        {
                            string filePath = $"{appSettings.FolderWritePath}\\{fileName}";
